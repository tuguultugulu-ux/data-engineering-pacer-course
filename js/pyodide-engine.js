...
`;
        ...
        // 5. Exact Output Match & Hardcode Cheat Prevention
        if (challengeMeta && challengeMeta.solution) {
            try {
                pyodide.globals.set('_solution_code', challengeMeta.solution);
                const solutionRunner = `
import sys, io
_old_stdout = sys.stdout
sys.stdout = io.StringIO()
try:
    exec(_solution_code, {})
    _sol_out = sys.stdout.getvalue()
except Exception as e:
    _sol_out = str(e)
finally:
...
_sol_out.strip()
`;
                const expectedStdout = await pyodide.runPythonAsync(solutionRunner);
                const userStdout = (execRes.stdout || "").trim();
                
                // Check if they just hardcoded a giant print statement
                const cheatCheckScript = `
import ast
try:
    tree = ast.parse(_cell_code_to_run)
    # Get all string literals in the code
    strings = [getattr(n, 's', getattr(n, 'value', '')) for n in ast.walk(tree) if isinstance(n, ast.Constant) and isinstance(getattr(n, 'value', None), str)]
    # Check if any string literal contains a large portion of the expected output
    is_cheating = any(len(s) > 20 and s in _expected_out for s in strings if type(s) == str)
except Exception:
    is_cheating = False
is_cheating
`;
                pyodide.globals.set('_expected_out', expectedStdout);
                const isCheating = await pyodide.runPythonAsync(cheatCheckScript);

                if (isCheating) {
                    tests.push({
                        name: isMn ? "Гаралтын Хуурамч Утга (Cheat Check)" : "Hardcoded Output Detection",
                        passed: false,
                        msg: isMn ? "Алдаа: Хүлээгдэж буй үр дүнг шууд хэвлэсэн байна. Кодоо бодит байдлаар бичнэ үү." : "Failed: Detected hardcoded expected output strings. You must compute the result programmatically."
                    });
                } else if (userStdout === expectedStdout) {
                    tests.push({
                        name: isMn ? "Гаралтын Үр Дүнгийн Баталгаажуулалт" : "Output Consistency Validation",
                        passed: true,
                        msg: isMn ? "Кодын гаралт хүлээгдэж буй үр дүнтэй яг таарч байна." : "Standard output matches the exact expected solution output."
                    });
                } else {
                    tests.push({
                        name: isMn ? "Гаралтын Үр Дүнгийн Баталгаажуулалт" : "Output Consistency Validation",
                        passed: false,
                        msg: isMn ? "Гаралт хүлээгдэж буй үр дүнгээс зөрж байна. Даалгаврын шаардлагыг дахин шалгана уу." : "Failed: The output generated by your code does not match the expected result."
                    });
                }
            } catch (err) {
                console.warn("Output validation error:", err);
            }
        }
