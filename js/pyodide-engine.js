...
`;
        ...
        // 5. Structural & Output Validation (Flexible for different coding styles)
...
_sol_locals = {}
try:
    exec(_solution_code, globals(), _sol_locals)
...
_sol_dfs = [v for v in _sol_locals.values() if isinstance(v, pd.DataFrame)]
_sol_shape = _sol_dfs[0].shape if _sol_dfs else None
json.dumps({"stdout": _sol_out.strip(), "shape": _sol_shape})
`;
                const expectedJson = await pyodide.runPythonAsync(solutionRunner);
                const expectedData = JSON.parse(expectedJson);
                const expectedStdout = expectedData.stdout;
                const expectedShape = expectedData.shape;
                
                const userStdout = (execRes.stdout || "").trim();
                
                // Cheat Check: Ensure they didn't just hardcode the final answer string
...
`;
                ...
                if (isCheating) {
                    tests.push({
                        name: isMn ? "Хуурамч Үр Дүн (Cheat Check)" : "Hardcoded Output Detection",
                        passed: false,
